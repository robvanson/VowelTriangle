...
OutputHeight <- 7.5;
#
filename <- "CorpusConcatArea_plot.pdf"
...
plot(as.numeric(AverageAllVowels$Style)+1, AverageAllVowels$Area2, type="b", col=colorlist[3], pch=21, ylab="Vowel space area (2SD)", cex.lab=2.5, xlab="Speaking style", xlim=c(0.95,6.05), ylim=c(51,145), bg="black", lty=2, lwd=4, axes=FALSE, frame.plot=TRUE)
text(x=3.5, y=140, label="Concatenated", cex=2.5, pos=3)
...
text(x=c(2:6), y=52, pos=1, cex=1.5, labels=aggregate(NSpeakerVowels$N~Style, data=NSpeakerVowels, mean)[,2])

# All the speakers
epsilon <- 0.03
for(i in 1:length(SpeakerList)){
	xSpeaker <- as.numeric(AverageSpeakerVowels[AverageSpeakerVowels$Speaker==SpeakerList[[i]],]$Style)+1
...
# Fill in the labels
lines(as.numeric(AverageAllVowels$Style)+1, AverageAllVowels$Area2, type="p", fg=colorlist[3], col=colorlist[3], pch=21, ylab="Vowel space area (2SD)", xlab="Speaking style", xlim=c(0.1, 6.1), ylim=c(55,145), bg="white", lty=2, lwd=6, cex=3)
# All the speakers
for(i in 1:length(SpeakerList)){
	xSpeaker <- as.numeric(AverageSpeakerVowels[AverageSpeakerVowels$Speaker==SpeakerList[[i]],]$Style)+1
	ySpeaker <- AverageSpeakerVowels[AverageSpeakerVowels$Speaker==SpeakerList[[i]],]$Area2
	...
	reAdj <- runif(1, 0, 1)
	lines(as.numeric(AverageSpeakerVowels[AverageSpeakerVowels$Speaker==SpeakerList[[i]],]$Style)+1, AverageSpeakerVowels[AverageSpeakerVowels$Speaker==SpeakerList[[i]],]$Area2, type="p", col=colorlist[[i]], pch=SpeakerList[[i]], cex=1)
...
lines(as.numeric(AverageAllVowels$Style)+1, AverageAllVowels$Area2, type="p", fg=colorlist[3], col=colorlist[3], pch=21, ylab="Vowel space area (2SD)", xlab="Speaking style", xlim=c(0.95,6.05), ylim=c(55,145), bg="white", lty=2, lwd=6, cex=3)
