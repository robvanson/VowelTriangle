...
#for(fm in c("FM", "VTL")){
#	for(analysis in c("SL", "Burg", "Robust")){
for(fm in c("VTL")){
	for(analysis in c("SL")){
		print("", quote=FALSE)
		print(paste("Determine speaker sex: ", fm, sep=""), quote=FALSE)
		print(paste("Formant analysis: ", analysis, sep=""), quote=FALSE)
		
		file <- paste("IFA_concat_data", fm, "_", analysis, ".tsv", sep="")
		VowelTable <- read.table(file, header = TRUE, sep = "\t", na.strings = "-");
		...
		# Leave one out tests
		print("", quote=FALSE)
		print("Leave-One-Out predictions of linear models: Concat", quote=FALSE)
		speakerList <- unique(VowelTable$Speaker)
		styleList <- unique(VowelTable$Style)
			...
			for(style in styleList){
				for(session in sessionList){
					...
					for(chunkName in chunkList){
						...
						model <- lm(Area2 ~ Speaker*Style + Speaker*Session + Style*Session, trainTable)
						predArea <- predict(model, testTable)
